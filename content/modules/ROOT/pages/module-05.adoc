...
=== Lab 1: AIOps with Apache Remediation
...
* Used Red Hat Lightspeed (formerly Insights) to identify CVEs and Advisor recommendations across a RHEL fleet
* Generated remediation playbooks from Lightspeed for vulnerabilities, misconfigurations, and System Roles
* Used Automation code assistant (with Gemini backend) to generate an HTTPD fix in the IDE
* Reviewed AI-generated playbooks in code-server (VS Code) before committing to Git
* Used Claude Code with the Ansible MCP server to interactively discover servers needing patches
...
* Compared human-in-the-loop (Lab 4) vs fully autonomous (Lab 5) approaches
...
| `🛡️ CVE Remediation` | JT patches a specific CVE identified by Red Hat Lightspeed
| `📋 Advisor Remediation` | JT fixes an Advisor-flagged misconfiguration from Red Hat Lightspeed
...
== Lab 2: Network Automation Playbooks

=== Network-AIOps-Workflow Overview

The network automation portion of this lab follows the same AIOps principles as Lab 1, but applies them to Cisco router OSPF troubleshooting scenarios.
...
=== Lightspeed Remediation Overview

Lab 4 introduced a developer-centric approach to AIOps. Instead of fully automated playbook generation via API (as in Labs 1-3), this workflow brings the human into the loop through code-server (VS Code) and Claude Code.

=== Red Hat Lightspeed Playbook Generation

*Purpose:*
Use Red Hat Lightspeed (formerly Insights) to identify CVEs and Advisor recommendations, then generate targeted remediation playbooks.

*What it does:*

- Scans connected RHEL systems for vulnerabilities (CVEs) and misconfigurations (Advisor)
- CVE playbooks are downloaded agentically via Claude Code and the Lightspeed MCP server
- Advisor playbooks are generated manually through the Lightspeed web UI (MCP support coming in a future release)
- HTTPD fix is generated using Automation code assistant (formerly Ansible Lightspeed Code Assistant) with Gemini backend

*Four playbooks generated:*

1. **CVE Remediation** — patches a specific vulnerability on `{rhel_node2}` (agentic via MCP)
2. **Advisor Remediation** — fixes a Red Hat-flagged misconfiguration on `{rhel_node3}` (manual)
3. **HTTPD Configuration Fix** — generated by Automation code assistant (Gemini) on `{rhel_node1}`
4. **System Roles (NTP)** — pre-built playbook using `rhel-system-roles.timesync`

=== Agentic IDE Workflow

*Purpose:*
Demonstrate human-in-the-loop agentic automation using Claude Code in code-server, connected to AAP via the Ansible MCP server.

*What it does:*

- Student prompts Claude Code in natural language (e.g., "Are there servers that need patching?")
- Claude Code queries AAP inventory via MCP to discover hosts and their state
- Claude Code identifies available Job Templates that match the remediation need
- Claude Code asks for explicit permission before executing any action
- Upon approval, Claude Code launches the Job Template through AAP
...
Lab 5 demonstrated the end state of AIOps maturity: fully autonomous self-healing. This workflow uses the same Splunk infrastructure from Lab 2 (Network Automation) but applies it to RHEL system monitoring.
...
*Key principle:* This Job Template was reviewed and approved during Lab 4. Its promotion to autonomous execution reflects organizational trust in tested, validated automation.

=== The AIOps Maturity Spectrum

[cols="1,2,2", options="header"]
|===
| Maturity Level | Approach | Workshop Lab
