= Module 1.1 — Log Enrichment and Prompt Generation Workflow

[NOTE]
====
**Terminology Note**: Red Hat has recently consolidated its AI-powered services under new branding. Throughout Parts 1-3, you will see references to **Ansible Lightspeed** — this technology is now called **Automation Code Assistant** and supports multiple AI backends (Gemini, Red Hat AI, or IBM watsonx). Similarly, **Red Hat Insights** on console.redhat.com is now branded as **Red Hat Lightspeed**. The functionality is the same — only the names have changed. You will work with the new naming in Parts 4 and 5.
====
...
[quote]
💡 **Want to deploy your own AI infrastructure?** Red Hat AI can be deployed using Ansible Content Collections like `redhat.ai` (Certified) and `infra.ai` (Validated), available as part of your AAP subscription. These collections provide a validated path to deploy generative AI models using automation. For this lab, we're using a managed service to simplify the setup, but the same workflow concepts apply whether you're using a managed service or self-hosted AI infrastructure.
