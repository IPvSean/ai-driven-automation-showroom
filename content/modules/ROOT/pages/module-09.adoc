...
This module uses a different EDA rulebook activation than the OSPF network module (Part 2). Both the OSPF and Cron rulebooks listen for Splunk webhooks on the same port (5000), so only one can be active at a time.
...
image::iknow_aiops.gif[I know AIOps, link="_images/iknow_aiops.gif", window=blank]
