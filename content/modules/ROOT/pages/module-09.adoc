...
In earlier parts of this lab, you used AI to *generate* automation. Red Hat AI analyzed logs, Automation coding assistant wrote playbooks, and you reviewed the output before running it. The AI was powerful, but it operated in a fixed pipeline: detect, analyze, generate, execute.
...
2. Enter your password if prompted: `{ssh_password}`
...
+
image::eda_ai_agent_running.png[AI Agent Activation Running,60%, link="_images/eda_ai_agent_running.png", window=blank]
...
image::mattermost_agent_report.png[Mattermost AI Agent Report,100%, link="_images/mattermost_agent_report.png", window=blank]

The report should show a *green* bar on the left if the issue was resolved, or a *red* bar if it was not.

////
=== Step 3.5: Test the agent's judgment with a problem it cannot fix
