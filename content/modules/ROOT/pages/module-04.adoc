= Section 2.2 - Event Driven Ansible: Splunk Webhook Setup
...
* Understand the Observe-Evaluate-Respond feedback loop in Event-Driven Ansible
* Verify that EDA rulebook activations are running correctly
* Review rulebook configurations for webhook listeners
* Explain how EDA integrates with observability tools like Splunk
* Prepare EDA to automatically trigger workflows based on network events
...
[quote]
💡 **Tip:** You can also view the complete rulebook source code in the GitHub repository at https://github.com/ansible-tmm/aiops-summitlab/blob/main/rulebooks/ospf.yml[github.com/ansible-tmm/aiops-summitlab/rulebooks/ospf.yml^]. All playbooks and rulebooks used in this lab are available there.
...
You have verified the EDA controller configuration. In the next section, you'll see the complete AI-driven network automation workflow in action, from OSPF failure detection in Splunk, through Event-Driven Ansible triggering, to Red Hat AI generating the remediation playbook automatically.
