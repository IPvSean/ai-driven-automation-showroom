= Section 1.1 - Log Enrichment and Prompt Generation Workflow

[NOTE]
====
**Terminology Note**: Red Hat has recently updated its AI-powered services under new branding. Throughout Parts 1-3, you will see references to **Ansible Lightspeed** in job template and workflow names within Ansible Automation Platform. This technology is now called **Automation coding assistant** and supports multiple AI backends (Gemini, Red Hat AI, or IBM watsonx). The Lightspeed brand is now reserved for the **Red Hat Lightspeed** platform on console.redhat.com (formerly Red Hat Insights). The functionality is the same; only the names have changed. You will work with the new naming in Part 4.
====
...
This workflow will check the status of the Apache service, notify a Mattermost channel with details, ask Red Hat AI to analyze the incident, and finally create a new job template using the AI-generated prompt for Automation coding assistant.
...
NOTE: Enabling **Prompt on Launch** for Extra Variables is required so that Event-Driven Ansible can pass event data (`ansible_eda` variables) to the workflow when it triggers automatically.
...
- A prompt ready for Automation coding assistant to generate the fix
...
* Prepares an Automation coding assistant prompt for automated playbook generation
