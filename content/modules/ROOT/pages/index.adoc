...
- *Inference* - Inference in AI refers to the process of using a trained model to make predictions or decisions based on new, unseen data.  For this AIOps workflow we are using *Red Hat AI* for understanding service issues like an application outage, and *Automation coding assistant* to create an Ansible Playbook.
...
- Automation coding assistant to generate a remediation playbook
...
- Automation coding assistant (formerly Ansible Lightspeed) for AI-powered playbook generation in the IDE
...
* Gitea - Git service used to host the AI-generated playbook.
...
* Red Hat AI & Automation coding assistant - AI services that analyze logs and generate remediation content.
...
Sections 1, 2, 3 can run concurrently since their activations do not conflict. Sections 5 and 6 require toggling activations as noted. Section 4 (Red Hat Lightspeed and Agentic IDE Workflow) does not use EDA at all.
====
...
| A basic introduction to AIOps: build a self-healing workflow for Apache service failures using Red Hat AI and Automation coding assistant. (30-40 min)

| xref:module-03.adoc[Section 2]
| xref:module-03.adoc[AIOps with Network Automation]
| Apply the same AIOps patterns to network infrastructure. Troubleshoot OSPF routing issues on Cisco routers using Splunk, Event-Driven Ansible, and AI-generated playbooks. (30-40 min)
...
| Use Red Hat Lightspeed to generate CVE and Advisor remediation playbooks, use Automation coding assistant to generate an HTTPD fix in code-server, then use Claude Code to interactively discover and remediate issues via the Ansible MCP server. (50-70 min)
...
3. *Remediation Workflow* - This workflow will focus on automatically creating an Ansible Playbook to resolve the issue by using Automation coding assistant.  This workflow will take the prompt created by the previous workflow, request a playbook from Automation coding assistant, sync this playbook to Git and then automatically create a Job Template for us to remediate the issue.
