...
This workshop consists of five independent parts:

- *Part 1: AIOps with Apache Remediation* - 30-40 minutes
- *Part 2: Network Automation* - 30-40 minutes
- *Part 3: Windows Automation* - 15-20 minutes
- *Part 4: Red Hat Lightspeed & Agentic IDE Workflow* - 50-70 minutes
- *Part 5: Autonomous AIOps with Splunk and RHEL* - 20-30 minutes
- *Total Workshop Time* - ~150-200 minutes

For each workshop, there are 4 parts:
...
* For part 1: *Execute HTTPD Remediation* - This is the final Job Template that will actually fix the application outage on the RHEL webserver.
* For part 2: *AI Driven Ansible for Network Automation* - This lab synthesizes the AI skills from the preceding sections of the {lab_name} into a practical network automation scenario.
...
[NOTE]
====
**This lab has two independent parts!** You can complete Part 1, Part 2, or both—in any order. If you've already completed Part 1 and want to explore a more advanced scenario, feel free to skip ahead to Part 2.
====
...
[IMPORTANT]
====
**Highly Recommended!** While Part 2 can be completed independently, we strongly encourage completing this section. Part 1 provides a **basic introduction** to AIOps concepts using Apache service remediation. Part 2 applies the same AIOps principles to a **more complex, real-world networking scenario**—demonstrating how AI-driven automation scales to handle sophisticated infrastructure challenges.
====

Part 2 applies the same three AIOps pillars you learned in Part 1—**Observability**, **Inference**, and **Automation**—to network infrastructure:
...
This final part extends the AIOps pattern to the Windows ecosystem, demonstrating that the same observe-analyze-respond workflow applies universally across all platforms.

Part 3 shows you how Event-Driven Ansible can detect Windows security events and automatically create intelligent tickets. The Windows environment includes:
...
* **Kafka** - Centralized event bus (shared infrastructure with Parts 1 and 2)
...
* **Part 1 (Linux)**: Filebeat → Kafka → EDA → AAP (detects service failures)
* **Part 2 (Network)**: Splunk → Webhook → EDA → AAP (detects OSPF issues)
* **Part 3 (Windows)**: Winlogbeat → Kafka → EDA → AAP (detects security events)
* **Part 4 (Lightspeed + IDE)**: Red Hat Lightspeed → code-server + Claude Code → Human reviews → Git → AAP (human-in-the-loop with AI agent)
* **Part 5 (Autonomous)**: Splunk → Webhook → EDA → AAP (fully autonomous self-healing)
...
Part 4 introduces two new dimensions to AIOps: **Red Hat Lightspeed** (formerly Insights) for vulnerability and compliance management, and **Claude Code** as an AI agent in **code-server (VS Code)** for interactive infrastructure management.
...
This final part demonstrates the **end state of AIOps maturity**: fully autonomous self-healing infrastructure. Splunk monitors RHEL systems for misconfigurations, and when one is detected, pre-approved automation fixes it — without any human intervention.
...
* **Parts 1-3**: Event-driven with AI content generation and human review checkpoints
* **Part 4**: Human-in-the-loop with an AI agent that asks permission before every action
* **Part 5**: Fully autonomous — pre-approved, trusted automation handles everything
...
**Part 2: AIOps with Network Automation** (Modules 3-5) Highly Recommended
...
**Part 3: AIOps with Windows Automation** (Module 6) Highly Recommended
...
**Part 4: Red Hat Lightspeed & Agentic IDE Workflow** (Modules 7-8) NEW
...
**Part 5: Autonomous AIOps with Splunk and RHEL** (Module 9) NEW
...
[TIP]
====
**Choose your path:** All five parts are independent and can be completed in any order. New to AIOps? Start with Part 1. Already familiar with the basics? Jump to Part 2 for network automation, Part 3 for Windows, Part 4 for Insights and agentic IDE workflows, or Part 5 for fully autonomous self-healing.
====
