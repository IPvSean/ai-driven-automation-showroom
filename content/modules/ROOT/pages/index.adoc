...
- Red Hat Insights for CVE and Advisor remediation playbook generation
- A developer IDE with an AI agent connected to AAP via the Ansible MCP server
...
- *Part 4: Red Hat Insights & Agentic IDE Workflow* - 50-70 minutes
...
* **Part 4 (Insights + IDE)**: Red Hat Insights → IDE → Human reviews → Git → AAP (human-in-the-loop with AI agent)
...
Part 4 introduces two new dimensions to AIOps: **Red Hat Insights** for vulnerability and compliance management, and an **AI agent in a developer IDE** for interactive infrastructure management.

[IMPORTANT]
====
**These are two separate demos** that work together:

. **Red Hat Insights** (Section 7): Generate remediation playbooks for CVEs and Advisor recommendations, review them in an IDE, and push to Git/AAP
. **Agentic IDE Workflow** (Section 8): Use an AI agent with the Ansible MCP server to discover servers needing patches, find matching Job Templates, and execute remediation — with human approval at every step
====

* **Observability**: Red Hat Insights continuously monitors RHEL systems for CVEs and misconfigurations
* **Inference**: Insights generates targeted remediation playbooks; the IDE agent reasons about what actions are needed
* **Automation**: Playbooks are pushed to Git and promoted to AAP Job Templates; the agent executes them with human approval

=== What You'll Learn

* How to use Red Hat Insights to generate CVE and Advisor remediation playbooks
* Reviewing AI-generated playbooks in a developer IDE before committing
* Using an AI agent with the Ansible MCP server to interactively manage RHEL infrastructure
...
**Part 4: Red Hat Insights & Agentic IDE Workflow** (Modules 7-8) NEW

Use **Red Hat Insights** to generate remediation playbooks for CVEs and Advisor recommendations, then use an **AI agent in a developer IDE** to interactively discover and remediate issues via the Ansible MCP server.
