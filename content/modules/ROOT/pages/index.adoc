...
. **Red Hat Lightspeed** (Section 7): Generate remediation playbooks for CVEs and Advisor recommendations, use Automation Code Assistant (formerly Ansible Lightspeed Code Assistant) to generate an HTTPD fix, review everything in code-server, and push to Git/AAP
. **Agentic IDE Workflow** (Section 8): Use Claude Code with the Ansible MCP server to discover servers needing patches, find matching Job Templates, and execute remediation — with human approval at every step
====
